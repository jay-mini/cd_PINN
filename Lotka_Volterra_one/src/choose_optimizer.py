...
def AdaHessian(model_param, lr=1.0, betas=(0.9, 0.999),
    ...
    optimizer = torch.Adahessian(model_param,
                                 lr=lr,
                                 betas=betas,
                                 eps=eps,
                                 weight_decay=weight_decay,
...
def Shampoo(model_param, lr=1e-1, momentum=0.0, weight_decay=0.0,
    ...
    """
    optimizer = torch.optim.Shampoo(model_param,
                                    lr=lr,
                                    momentum=momentum,
                                    weight_decay=weight_decay,
                                    epsilon=epsilon,
...
def Yogi(model_param, lr=1e-2, betas=(0.9, 0.999), eps=1e-3, initial_accumulator=1e-6,
         weight_decay=0):
    optimizer = torch.optim.Yogi(model_param,
                                 lr=lr,
                                 betas=betas,
                                 eps=eps,
...
def Apollo(model_param, lr=1e-2, beta=0.9, eps=1e-4, warmup=5, init_lr=0.01, weight_decay=0):
    ...
    optimizer = torch.optim.Apollo(model_param,
                                   lr=lr,
                                   beta=beta,
                                   eps=eps,
                                   warmup=warmup,
